...
function Trainer:train(epoch, dataloader)
   ...
   local nBatches = dataloader:size()
   ...
   for n, sample in dataloader:run() do
      ...
      if self.opt.lrShape == 'cosine' then
         self.optimState.learningRate = self:learningRateCosine(epoch, n, nBatches)
      end
      ...
      print((' | Epoch: [%d][%d/%d]   Time %.3f  Data %.3f  Err %1.3f  top1 %7.2f  top5 %7.2f  lr %.4f'):format(
         epoch, n, nBatches, timer:time().real, dataTime, loss, top1, top5, self.optimState.learningRate))
...
function Trainer:learningRate(epoch)
   ...
   local epoch_cur = (epoch-1) % nEpochs_cur + 1
   local decay = 0
   if self.opt.dataset == 'imagenet' then
      decay = math.floor((epoch_cur - 1) / nEpochs_cur * 3)
   else
      decay = epoch_cur >= 0.75*nEpochs_cur and 2 or epoch_cur >= 0.5*nEpochs_cur and 1 or 0
   end
